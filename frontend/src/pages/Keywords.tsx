...
import { keywordsApi, KeywordCreate, EnrichedKeywordOut } from '../services/api'
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      for (const row of dataRows) {
        ...
        if (trend3Idx >= 0 && cols[trend3Idx]) kwData.trend_3m = parseFloat(cols[trend3Idx]) || 0
      ...
      setUploadStatus(`⚠ Hata: ${e?.response?.data?.detail || e?.message || 'Bilinmeyen hata'}`)
    }
  }
    ...
    <div className="keywords-page animate-fade-in">
      <header className="page-header">
        ...
        <div className="header-actions">
          <button className="btn btn-secondary" onClick={fetchKeywords} disabled={loading}>
            <RefreshCw size={16} />
            Yenile
          </button>
        </div>
      ...
      <div className="tab-content">
        ...
        {!workspaceDisabled && activeTab === 'csv' && (
          <div className="upload-section glass-card">
            ...
            <p>
              CSV formatı: keyword, sector, monthly_volume, trend_12m, trend_3m, competition_score
            </p>
            ...
            <button className="btn btn-primary" onClick={handleUpload} disabled={!uploadedFile}>
              <Upload size={16} />
              Yükle
            </button>
          </div>
                ...
                {filteredKeywords.map((kw) => {
                  ...
                  return (
                    <tr key={kw.id}>
                      ...
                      <td>{competition != null ? competition.toFixed(2) : '—'}</td>
                      <td>
                        <button
