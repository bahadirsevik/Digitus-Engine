...
export default function Channels() {
  ...
  useEffect(() => {
    ...
    const defaultCoef = Number(
      selectedRunData.default_relevance_coefficient ?? 1,
    );
  ...
  const getButtonLabel = (run: ScoringRun): string => {
    if (!canAssign(run) && run.status === "scored")
      return "Önce İlgi Skoru Hesaplayın";
  ...
  return (
    <div className="channels-page animate-fade-in">
      ...
      {error && (
        <div className="channels-alert channels-alert-error">{error}</div>
      )}
        ...
        <div className="run-buttons">
          {runs.length === 0 ? (
            <p className="empty-text">
              Tamamlanmış skorlama çalışması bulunamadı.
            </p>
          ) : (
            runs.map((run) => (
              <button
          ...
          <div className="channels-controls">
            <div className="channels-coef-input">
              ...
              <input
                ...
                onChange={(e) =>
                  setRelevanceCoefficient(
                    normalizeCoefficient(Number(e.target.value)),
                  )
                }
              />
            </div>
